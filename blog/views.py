...
def category_blogs(request, slug):
    ...
    context = {"blogs": blogs, "tags": tags, "all_blogs": all_blogs}
    ...
    if request.method == "POST" and request.user.is_authenticated:
        ...
        if form.is_valid():
            Comment.objects.create(
                user=request.user, blog=blog, text=form.cleaned_data.get("text")
            )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            Reply.objects.create(
                user=request.user, comment=comment, text=form.cleaned_data.get("text")
            )
        ...
        if form.is_valid():
            ...
            for tag in tags:
                tag_input = Tag.objects.filter(
                    title__iexact=tag.strip(), slug=slugify(tag.strip())
                )
                if tag_input.exists():
                    t = tag_input.first()
                    blog.tags.add(t)
                    ...
                    if tag != "":
                        new_tag = Tag.objects.create(
                            title=tag.strip(), slug=slugify(tag.strip())
                        )
